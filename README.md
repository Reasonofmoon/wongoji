...
첫 화면의 **성능 실험용 원고**에서 예시를 골라 바로 돌려 볼 수 있다. 오류가 몇 건인지 사람이 세어 둔 글이라 앱이 무엇을 잡고 무엇을 놓치는지 직접 판단할 수 있다. `할머니 댁`은 **무오류 대조편**이라 부호가 앉는 자리가 곧 오탐이다. 자동 채점은 하지 않는다 — 정답 자리를 화면에 실으면 스스로 볼 기회가 사라진다.

첫 화면에서 학생 글을 붙여 넣거나 **원고지 사진을 올린다**. 사진은 칸 단위로 읽은 뒤 확인 화면에서 고치고 첨삭한다. 검토 화면에서 항목을 승인·수정·기각한 뒤 PNG 또는 학생용 PDF를 받는다.
...
- 원고지 사진 입력(OCR)은 비전 모델을 쓴다. 인식 결과를 **교사가 칸 단위로 확인해야** 첨삭이 돈다(`/api/ocr` → `/api/ocr/confirm` → `/api/chumsak`). 확인을 건너뛰는 경로는 서버가 409로 막는다. OCR은 맞춤법을 고치지 않는다 — 고치면 첨삭할 오류가 사라진다.
- OCR에는 LLM 키가 필요하다. 키가 없으면 사진 입력만 502로 실패하고 붙여넣기 경로는 그대로 돈다.
